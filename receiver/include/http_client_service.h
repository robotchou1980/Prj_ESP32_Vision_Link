...
#ifdef ENABLE_STREAM_MODE
    /**
     * @brief Fetch next JPEG frame from an MJPEG stream endpoint
     * @param url Complete stream URL (e.g., "http://192.168.1.100:80/stream")
     * @param buffer Output buffer for JPEG data
     * @param maxSize Maximum buffer size
     * @param timeoutMs Read timeout in milliseconds
     * @return Size of fetched frame, 0 if failed
     */
    virtual size_t fetchStreamJpegFrame(const char* url, uint8_t* buffer, size_t maxSize, uint32_t timeoutMs = 5000) = 0;
#endif
...
#ifdef ENABLE_STREAM_MODE
    bool beginStream(const char* url, uint32_t timeoutMs);
    void closeStream();
    bool readLine(WiFiClient* stream, char* line, size_t maxLen, uint32_t timeoutMs);
    bool readExact(WiFiClient* stream, uint8_t* buffer, size_t length, uint32_t timeoutMs);
    bool readStreamFrame(WiFiClient* stream, uint8_t* buffer, size_t maxSize, size_t& frameSize, uint32_t timeoutMs);
#endif
...
#ifdef ENABLE_STREAM_MODE
    size_t fetchStreamJpegFrame(const char* url, uint8_t* buffer, size_t maxSize, uint32_t timeoutMs = 5000) override;
#endif
